...
| **LLM Fallback 真实接入** | P14-01 | 评估选型：对比 OpenAI / Claude / 本地模型（Ollama/Llama）的印尼语能力、延迟、成本 | P0全量完成 |
| | P14-02 | 定义触发策略：低置信度阈值、连续未匹配计数、用户情绪异常检测等触发条件 | P14-01完成 |
| | P14-03 | 实现上下文组装器：对话历史摘要 + 用户画像 + 当前催收阶段 → LLM prompt 模板 | P14-02完成 |
| | P14-04 | 实现 LLM Interface：替换 mock provider，接入选定 LLM API，支持流式响应 | P14-03完成 |
| | P14-05 | 构建安全围栏：LLM 输出合规校验（敏感词过滤、话术规范检查、禁止表述拦截） | P14-04完成 |
| | P14-06 | 实现降级策略：LLM 超时/不可用时的多级降级（ML分类器 → 默认话术 → 转人工标记） | P14-04完成 |
| | P14-07 | 端到端集成测试：chatbot 接入 LLM fallback 全链路，验证触发→生成→校验→回复 | P14-05、P14-06完成 |
| | P14-08 | 效果评估：对比 LLM fallback vs 纯规则/ML 在边界场景下的成功率、合规率、延迟 | P14-07完成 |
...
graph TD
    ...
    BASE --> P14[P1-4 LLM Fallback<br>真实接入]
    P01 --> P14
