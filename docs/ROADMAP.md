...
| P1.5 效果优化 | **12%** (4/33) | P15-A02 沉默破解 ✅、P15-H01 LLM话术生成 ✅、P15-G01 校准基线 ✅、P15-A01 异议反驳链 ✅、29 项待启动 |
...
| 事项ID | 具体内容 | 前置依赖 | 优先级 |
|--------|----------|----------|--------|
| P15-B01 | **新客/老客差异化话术**：根据 `total_loan_cnt` / `paidoff_loan_cnt` 切换教育型/认同型/依赖型话术 | 基础依赖层就绪 | P0 |
...
| P15-G03 | **校准模型训练**：训练 Logistic Regression / XGBoost 模型，输入对话特征 → 输出预估还款概率，替代二分类成功/失败 | P15-G02 完成 | P1 |
| P15-G04 | **模拟器虚假承诺注入**：基于真实校准系数，为模拟器新增"口头承诺但不履约"行为，使评测预估更接近线上预期 | P15-G01 完成 | P1 |
