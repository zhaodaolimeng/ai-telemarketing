...
PROFILES: dict[str, StrategyProfile] = {
    ...
    "nf=0_S0": StrategyProfile(
        segment_key="nf=0_S0",
        segment_name="新客·S0·紧急干预型",
        approach="intervene",
        tone="firm",
        push_intensity=4,
        max_objections=2,
        extension_fee_ratio=0.20,        # 降低展期门槛
        extension_priority=True,          # 优先推展期
        partial_payment_offered=True,     # 主动提示部分还款
        max_push_rounds=4,
        consequence_emphasis=4,           # 强调后果
        education_emphasis=True,          # 仍需教育
        relationship_emphasis=False,
    ),
    ...
    "nf=1_H2": StrategyProfile(
        segment_key="nf=1_H2",
        segment_name="新转老·H2·维护型",
        approach="maintain",
        tone="neutral",
        push_intensity=1,                 # 最轻推（87.1%有效还款率）
        max_objections=4,
    ...
    "nf=2_H2": StrategyProfile(
        segment_key="nf=2_H2",
        segment_name="老客·H2·极轻型",
        approach="light",
        tone="neutral",
        push_intensity=1,                 # 几乎不需推（90.4%有效还款率）
        max_objections=5,                 # 极宽容
        extension_fee_ratio=0.30,
        extension_priority=False,
        partial_payment_offered=False,
        max_push_rounds=1,                # 只推一次
        consequence_emphasis=1,
        education_emphasis=False,
        relationship_emphasis=True,
    ),
    "nf=2_S0": StrategyProfile(
        segment_key="nf=2_S0",
        segment_name="老客·S0·强硬型",
        approach="firm",
        tone="urgent",
        push_intensity=5,                 # 最强推
        max_objections=2,
        extension_fee_ratio=0.35,         # 提高展期费用
        extension_priority=False,         # 优先全额，老客明知规则
        partial_payment_offered=True,
        max_push_rounds=5,                # 最大耐心 (53.4%有效)
        consequence_emphasis=5,           # 最强后果强调
        education_emphasis=False,
        relationship_emphasis=False,      # 不套近乎
    ),
}
...
def get_strategy_profile(
    ...
    Returns:
        StrategyProfile for the segment
