...
def main():
    ...
    for i, (pattern, cnt) in enumerate(success_patterns[:5], 1):
        print(f"\n模式{i} [{cnt}次]: {pattern}")

    # 失败对话常见模式
    print("\n失败对话典型模式:")
    failed_patterns = analyze_patterns(failed_samples)
    for i, (pattern, cnt) in enumerate(failed_patterns[:5], 1):
        print(f"\n模式{i} [{cnt}次]: {pattern}")
    ...
    # 保存结果
    output_file = Path("data/processed/final_analysis.json")
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "success_samples": [s["full_text"][:500] for s in success_samples[:5]],
            "failed_samples": [s["full_text"][:500] for s in failed_samples[:5]]
...
def analyze_patterns(samples):
    ...
    for s in samples:
        ...
        if "sudah" in text and "dibayar" in text:
            pattern_parts.append("确认已还款")
        if "jam" in text:
            pattern_parts.append("约定时间")
        if "ya" in text and "oke" in text:
            pattern_parts.append("确认用语")
        if "ngga" in text or "tidak" in text:
            pattern_parts.append("否定表达")
        if "selamat" in text:
            pattern_parts.append("礼貌问候")
        if "aplikasi" in text or "ekstra" in text:
            pattern_parts.append("提及应用")
        if "belum" in text:
            pattern_parts.append("状态描述")
