...
def extract_utterances(gold_dir: Path, max_per_intent: int = 200,
                       skip_texts: set = None) -> list[dict]:
    """提取所有客户话语，优先采样式未知意图

    Args:
        skip_texts: 已标注文本集合 (lowercase)，跳过重复避免重复 LLM 调用
    """
    ...
    print(f"Total customer utterances (after dedup): {len(unknown_utterances) + len(known_utterances)}")
...
def main():
    ...
    if output_path.exists():
        ...
        for s in existing.get("training_data", []):
            skip_texts.add(s["text"].strip().lower())
        print(f"  已标注文本(去重): {len(skip_texts)}")
    ...
    if output_path.exists():
        existing = json.loads(output_path.read_text())
        existing_data = existing.get("training_data", [])
        existing_meta = existing.get("meta", {})
        existing_texts = set(s["text"].strip() for s in existing_data)
        # 只添加新的（不在已有标签中的）
        truly_new = [s for s in new_training_data if s["text"].strip() not in existing_texts]
    else:
        truly_new = new_training_data

    merged_data = existing_data + truly_new
    output_data = {
        "meta": {
            "task": "P15-C01",
            "description": "LLM 精标注意图标签，用于训练 ML 分类器",
            "total_annotated": len(merged_data),
            "intent_classes": len(set(s["intent"] for s in merged_data)),
            "rounds": existing_meta.get("rounds", 1) + 1,
        },
        "intent_distribution": dict(Counter(s["intent"] for s in merged_data)),
        "training_data": merged_data,
    }
    output_path.write_text(json.dumps(output_data, ensure_ascii=False, indent=2))
    print(f"\n结果已保存: {output_path}")
    print(f"  本轮新增: {len(truly_new)} 条")
